...
const channelId = process.env.CHANNEL_ID; // ID do canal onde a embed será enviada
...
// Funções para criar embeds reutilizáveis
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isButton() && interaction.customId === 'generate_code') {
    const row = new MessageActionRow().addComponents(
      new MessageButton()
        .setCustomId('generate_code')
        .setLabel('Gerar Código')
    ...
    const lastProcessNumber =
      existingCodes.length > 0
        ? parseInt(existingCodes[existingCodes.length - 1].split('-')[1].split('_')[0])
        : 0;
    const processNumber = ('00' + (lastProcessNumber + 1)).slice(-3);
    let randomChars = generateRandomString(5);
    const codigo = '0' + year + month + day + '-' + processNumber + '_' + randomChars;
